...
class CorrespondenceNumberGenerator:
    ...
    def generate_number():
        ...
        last_correspondence_number = Number.objects.filter(correspondence__created_at__year=year).order_by('-number').first()
        if last_correspondence_number:
            new_sequence = last_correspondence_number.number + 1
        else:
            new_sequence = 10001
        number_obj = Number.objects.create(number=new_sequence)
